...
exports.createPost = async (req, res, next) => {
  ...
  const post = new Post({
    title: title,
    content: content,
    creator: req.userId, // mongoose will convert id to a string
    imageUrl: imageUrl
  });
  try {
    await post.save();
...
exports.deletePost = async (req, res, next) => {
  ...
  try {
    ...
    clearImage(post.imageUrl);
    const deletedPost = await Post.findOneAndRemove(postId);
    const user = await User.findById(req.userId);
...
const clearImage = (filePath) => {
  ...
  fs.unlink(deleteFilePath, (err) => console.log(err));
};
